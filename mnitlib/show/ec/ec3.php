...
<!-- ################################php section####################################################################### -->
<?php
$b_query = "SELECT * FROM books WHERE dept = 'EC' AND status = 'UNDER PROCESS'";
$b_result = mysqli_query($db,$b_query) or die("ERROR QUERYING DATABAse".mysqli_error($db));
?>
<!-- ####################################################################################################### -->

      <ul>
        <h1 class="title">ELECTRONICS AND COMMUNICATIONS UNDER-PROCESS PROPOSALS</h1>
...
?>
<html>   
<form action="procurred.php" method="POST">  
...
<td>Priority:<input  id="priority" name="priority" value=<?php echo($b_row['priority']);?> readonly></td>
 ...
 <input type="submit" value="PROCURRED"/></form>
</html>
<?php
}
?>
