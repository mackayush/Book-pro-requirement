...
<!-- ################################php section####################################################################### -->
<?php
$b_query = "SELECT * FROM books WHERE dept = 'CS' AND status = 'PROCURRED'";
$b_result = mysqli_query($db,$b_query) or die("ERROR QUERYING DATABAse".mysqli_error($db));
?>
<!-- ####################################################################################################### -->

      <ul>
        <h1 class="title">COMPUTER SCIENCE PROCURRED PROPOSALS</h1>
...
?>
<html>    
<form action="ontherack.php" method="POST">
...
<td>Priority:<input  id="priority" name="priority" value=<?php echo($b_row['priority']);?> readonly></td>
 ...
 <input type="submit" value="ON THE RACK"/></form>
</html>
<?php
}
?>
